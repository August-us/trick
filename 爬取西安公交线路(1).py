...
all_url = 'http://xian.8684.cn'  ##开始的URL地址
start_html = requests.get(all_url, headers=headers) 
# print (start_html.text)
Soup = BeautifulSoup(start_html.text, 'lxml')
all_a = Soup.find('div',class_='bus-layer depth w120').find_all('div',class_='list')[2].find_all('a')
Network_list = []
for a in all_a:
    ...
    all_a2 = Soup2.find('div',class_='cc-content').find('div',class_='list clearfix').find_all('a') # 既有id又有class的div不知道为啥取不出来，只好迂回取了
    for a2 in all_a2:
