...
def pro(x):
    import re
    a=[[0.0, 0.004, 0.008, 0.012, 0.016, 0.0199, 0.0239, 0.0279, 0.0319, 0.0359] ,
    ...
    x=abs(x)
    x=str(round(x,2)) if len(str(round(x,2)))==4 else str(round(x,2))+'0'*(4-len(str(round(x,2))))
    try:
        return a[int(x[0]+x[2])][int(x[3])]
    except IndexError:
        return 0.5
def probability(x,y=0):
    x_=pro(x)
    y_=pro(y)
    return y_-x_ if x*y>0 else y_+x_


if __name__=="__main__":
    print(probability(-1.285))
